...
export const createPodcast = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (user.length === 0) {
      ...
      if (!newUser) {
        throw new ConvexError('Failed to fetch newly created user')
      }
...
export const getPodcastBySearch = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Busca en descripción y título con dos queries distintas y combina:
    const descSearch = await ctx.db
      .query('podcasts')
      .withSearchIndex('search_body', (q) =>
    ...
    const titleSearchBody = await ctx.db
      .query('podcasts')
      .withSearchIndex('search_title', (q) =>
        q.search('podcastTitle', args.search)
      )
    ...
    for (const p of descSearch) combinedMap.set(p._id.toString(), p)
    for (const p of titleSearchBody) combinedMap.set(p._id.toString(), p)
